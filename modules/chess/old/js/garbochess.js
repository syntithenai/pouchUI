function GarboChess() {
	...
	function WhiteToMove(orientation) {
		var me = g_toMove >> 3;
		var otherColor = 8 - g_toMove; 
		console.log('whose move',me,otherColor,orientation);
		var retval;
		if (orientation) {
			if (me==1 && otherColor==0)  {
				retval= true;
			} else {
				retval= false;
			}
		...
		console.log('white to move',retval);
		return retval;
	}
	...
	return {
		// constants
		'moveflagCastleKing' : moveflagCastleKing,
		'moveflagCastleQueen' : moveflagCastleQueen,
		'moveflagEPC' : moveflagEPC,
		'moveflagPromotion' : moveflagPromotion,
		'piecePawn' : piecePawn,
		...
		'g_toMove' : g_toMove,
		'g_timeout' : g_timeout,
		// public functions used for worker
		'ResetGame':ResetGame,
		'InitializeFromFen':InitializeFromFen,
		'Search':Search,
		'FinishMoveLocalTesting':FinishMoveLocalTesting,
		'FinishPlyCallback':FinishPlyCallback,
		'MakeMove':MakeMove,
		'GetMoveFromString':GetMoveFromString,
		// the rest of the public functions
		'GetFen':GetFen,
		'InitializeFromFen':InitializeFromFen,
		'GetMoveSAN':GetMoveSAN,
...
self.onmessage = function (e) {
	try {
		...
		if (e.data.match("^position") == "position") {
			gc.ResetGame();
			var result = gc.InitializeFromFen(e.data.substr(9, e.data.length - 9));
			if (result.length != 0) {
				postMessage("message " + result);
			}
		} else if (e.data.match("^search") == "search") {
			g_timeout = parseInt(e.data.substr(7, e.data.length - 7), 10);
			gc.Search(gc.FinishMoveLocalTesting, 99, gc.FinishPlyCallback);
		} else if (e.data == "analyze") {
			g_timeout = 99999999999;
			gc.Search(null, 99, gc.FinishPlyCallback);
		} else if (e.data == "getfen") {
			postMessage("fen "+gc.GetFen());
		} else {
			gc.MakeMove(gc.GetMoveFromString(e.data));
		}
	} catch (error) {
		postMessage('log '+JSON.stringify(error))
	}
}
